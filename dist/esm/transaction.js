import { decodeIdlAccount, fetchAccountDataById, findAta, findMintMetadataId, getBatchedMultipleAccounts, METADATA_PROGRAM_ID, tryDecodeIdlAccount, tryGetAccount, tryNull, withFindOrInitAssociatedTokenAccount, } from "@cardinal/common";
import { findMintManagerId as findCCSMintManagerId, findRulesetId, PROGRAM_ADDRESS, } from "@cardinal/creator-standard";
...
import { ComputeBudgetProgram, Keypair, SystemProgram, SYSVAR_INSTRUCTIONS_PUBKEY, SYSVAR_RENT_PUBKEY, } from "@solana/web3.js";
import { timeInvalidator, tokenManager, useInvalidator } from "./programs";
import { CLAIM_APPROVER_IDL, claimApproverProgram, defaultPaymentManagerId, } from "./programs/claimApprover";
...
import { CRANK_KEY, InvalidationType, TOKEN_MANAGER_ADDRESS, TOKEN_MANAGER_IDL, TokenManagerKind, tokenManagerProgram, TokenManagerState, } from "./programs/tokenManager";
import { getTokenManager } from "./programs/tokenManager/accounts";
import { findMintCounterId, findMintManagerId, findReceiptMintManagerId, findTokenManagerAddress, tokenManagerAddressFromMint, } from "./programs/tokenManager/pda";
import { getRemainingAccountsForClaim, getRemainingAccountsForIssue, getRemainingAccountsForKind, getRemainingAccountsForTransfer, getRemainingAccountsForUnissue, withRemainingAccountsForInvalidate, withRemainingAccountsForReturn, } from "./programs/tokenManager/utils";
import { getListing, getTransferAuthorityByName, } from "./programs/transferAuthority/accounts";
 ...
 */
export const withIssueToken = async (transaction, connection, wallet, { claimPayment, timeInvalidation, useInvalidation, mint, issuerTokenAccountId, amount = new BN(1), transferAuthorityInfo, kind = TokenManagerKind.Managed, invalidationType = InvalidationType.Return, visibility = "public", permissionedClaimApprover, receiptOptions = undefined, customInvalidators = undefined, rulesetId = undefined, }, payer = wallet.publicKey) => {
    ...
    if (transferAuthorityInfo) {
        const checkTransferAuthority = await tryGetAccount(() => getTransferAuthorityByName(connection, transferAuthorityInfo.transferAuthorityName));
    ...
    if (useInvalidation) {
        ...
        const useInvalidatorIx = await usgInvalidatorProgram.methods
            ...
            paymentManager: useInvalidation.paymentManager || defaultPaymentManagerId,
            ...
            maxUsages: ((_g = useInvalidation.extension) === null || _g === void 0 ? void 0 : _g.maxUsages)
                ? new BN((_h = useInvalidation.extension) === null || _h === void 0 ? void 0 : _h.maxUsages)
                : null,
            ...
            extensionPaymentAmount: ((_j = useInvalidation.extension) === null || _j === void 0 ? void 0 : _j.extensionPaymentAmount)
                ? new BN((_k = useInvalidation.extension) === null || _k === void 0 ? void 0 : _k.extensionPaymentAmount)
                : null,
        ...
        const useInvalidatorId = useInvalidator.pda.findUseInvalidatorAddress(tokenManagerId);
    ...
    // issuer
    const tokenManagerTokenAccountId = await withFindOrInitAssociatedTokenAccount(transaction, connection, mint, tokenManagerId, payer, true);
    const issueIx = await tmManagerProgram.methods
        ...
        .remainingAccounts(getRemainingAccountsForIssue(kind, mint, issuerTokenAccountId, tokenManagerTokenAccountId, rulesetId))
        .instruction();
    ...
    if (receiptOptions) {
        ...
        const receiptMintMetadataId = findMintMetadataId(receiptMintKeypair.publicKey);
        const recipientTokenAccountId = await findAta(receiptMintKeypair.publicKey, wallet.publicKey);
 ...
 */
export const withClaimToken = async (transaction, connection, wallet, tokenManagerId, additionalOptions, buySideTokenAccountId) => {
    var _a;
    ...
    const claimApproverData = claimApproverInfo
        ? tryDecodeIdlAccount(claimApproverInfo, "paidClaimApprover", CLAIM_APPROVER_IDL)
        : null;
    ...
    const claimReceiptId = tokenManager.pda.findClaimReceiptId(tokenManagerId, wallet.publicKey);
    ...
    if ((claimApproverData === null || claimApproverData === void 0 ? void 0 : claimApproverData.parsed) &&
        ...
        const payerTokenAccountId = getAssociatedTokenAddressSync(claimApproverData.parsed.paymentMint, wallet.publicKey);
        const [issuerTokenAccountId, feeCollectorTokenAccountId, remainingAccounts,] = await withRemainingAccountsForPayment(transaction, connection, wallet, tokenManagerData.parsed.mint, claimApproverData.parsed.paymentMint, tokenManagerData.parsed.issuer, claimApproverData.parsed.paymentManager, buySideTokenAccountId, {
    ...
    const tokenManagerTokenAccountId = getAssociatedTokenAddressSync(tokenManagerData.parsed.mint, tokenManagerId, true);
    const recipientTokenAccountId = getAssociatedTokenAddressSync(tokenManagerData.parsed.mint, wallet.publicKey);
    transaction.add(createAssociatedTokenAccountIdempotentInstruction((_a = additionalOptions === null || additionalOptions === void 0 ? void 0 : additionalOptions.payer) !== null && _a !== void 0 ? _a : wallet.publicKey, recipientTokenAccountId, wallet.publicKey, tokenManagerData.parsed.mint));
    // claim
    const claimIx = await tokenManagerProgram(connection, wallet)
        ...
        .remainingAccounts(getRemainingAccountsForClaim({ parsed: tokenManagerData.parsed, pubkey: tokenManagerId }, recipientTokenAccountId, metadata, claimReceiptId))
        .instruction();
...
export const withUnissueToken = async (transaction, connection, wallet, mintId) => {
    ...
    const [tokenManagerInfo, metadataInfo] = await getBatchedMultipleAccounts(connection, [tokenManagerId, findMintMetadataId(mintId)]);
    const metadata = metadataInfo
        ? Metadata.deserialize(metadataInfo.data)[0]
        : null;
    if (!tokenManagerInfo)
        throw "Token manager not found";
    const tokenManager = decodeIdlAccount(tokenManagerInfo, "tokenManager", TOKEN_MANAGER_IDL);
    transaction.add(createAssociatedTokenAccountIdempotentInstruction(wallet.publicKey, getAssociatedTokenAddressSync(mintId, wallet.publicKey), wallet.publicKey, mintId));
    transaction.add(await tokenManagerProgram(connection, wallet)
        .methods.unissue()
        .accounts({
        tokenManager: tokenManagerId,
        tokenManagerTokenAccount: getAssociatedTokenAddressSync(mintId, tokenManagerId, true),
        issuer: wallet.publicKey,
        issuerTokenAccount: getAssociatedTokenAddressSync(mintId, wallet.publicKey),
        tokenProgram: TOKEN_PROGRAM_ID,
    })
        .remainingAccounts(getRemainingAccountsForUnissue(tokenManagerId, tokenManager.parsed, metadata))
        .instruction());
    return transaction;
};
export const withInvalidate = async (transaction, connection, wallet, mintId, UTCNow = Date.now() / 1000) => {
    ...
    const tokenManagerId = tokenManagerAddressFromMint(mintId);
    const useInvalidatorId = useInvalidator.pda.findUseInvalidatorAddress(tokenManagerId);
    const timeInvalidatorId = timeInvalidator.pda.findTimeInvalidatorAddress(tokenManagerId);
    const [useInvalidatorData, timeInvalidatorData, tokenManagerData, metadata] = await Promise.all([
        ...
        tryNull(Metadata.fromAccountAddress(connection, findMintMetadataId(mintId))),
    ]);
    if (!tokenManagerData)
        return transaction;
    if (tokenManagerData.parsed.kind === TokenManagerKind.Programmable ||
    ...
    const recipientTokenAccount = await getAccount(connection, tokenManagerData.parsed.recipientTokenAccount);
    ...
    const remainingAccounts = await withRemainingAccountsForInvalidate(transaction, connection, wallet, mintId, tokenManagerData, recipientTokenAccount.owner, metadata);
    if (useInvalidatorData &&
...
export const withReturn = async (transaction, connection, wallet, tokenManagerData) => {
    ...
    const transferAccounts = getRemainingAccountsForKind(tokenManagerData.parsed.mint, tokenManagerData.parsed.kind);
    const invalidateIx = await tmManagerProgram.methods
        .invalidate()
        .accounts({
...
export const withUse = async (transaction, connection, wallet, mintId, usages, collector) => {
    ...
    if (!useInvalidatorData) {
        ...
        const initIx = await usgInvalidatorProgram.methods
            .init({
            collector: collector !== null && collector !== void 0 ? collector : CRANK_KEY,
...
export const withExtendExpiration = async (transaction, connection, wallet, tokenManagerId, secondsToAdd, options, buySideTokenAccountId) => {
    const tmeInvalidatorProgram = timeInvalidatorProgram(connection, wallet);
    const timeInvalidatorId = timeInvalidator.pda.findTimeInvalidatorAddress(tokenManagerId);
    ...
    if (timeInvalidatorData && timeInvalidatorData.parsed.extensionPaymentMint) {
        ...
        const [paymentTokenAccountId, feeCollectorTokenAccountId, remainingAccounts,] = await withRemainingAccountsForPayment(transaction, connection, wallet, tokenManagerData.parsed.mint, timeInvalidatorData.parsed.extensionPaymentMint, tokenManagerData.parsed.issuer, timeInvalidatorData.parsed.paymentManager, buySideTokenAccountId, {
            receiptMint: tokenManagerData.parsed.receiptMint,
...
export const withExtendUsages = async (transaction, connection, wallet, tokenManagerId, usagesToAdd, options, buySideTokenAccountId) => {
    const usgInvalidatorProgram = useInvalidatorProgram(connection, wallet);
    const useInvalidatorId = useInvalidator.pda.findUseInvalidatorAddress(tokenManagerId);
    const [useInvalidatorData, tokenManagerData] = await Promise.all([
    ...
    if (useInvalidatorData && useInvalidatorData.parsed.extensionPaymentMint) {
        ...
        const [paymentTokenAccountId, feeCollectorTokenAccountId, remainingAccounts,] = await withRemainingAccountsForPayment(transaction, connection, wallet, tokenManagerData.parsed.mint, useInvalidatorData.parsed.extensionPaymentMint, tokenManagerData.parsed.issuer, useInvalidatorData.parsed.paymentManager, buySideTokenAccountId, {
            receiptMint: tokenManagerData.parsed.receiptMint,
            payer: options === null || options === void 0 ? void 0 : options.payer,
        });
...
export const withUpdateMaxExpiration = async (transaction, connection, wallet, tokenManagerId, newMaxExpiration) => {
    const tmeInvalidatorProgram = timeInvalidatorProgram(connection, wallet);
    const timeInvalidatorId = timeInvalidator.pda.findTimeInvalidatorAddress(tokenManagerId);
    const [tokenManagerData] = await Promise.all([
        tokenManager.accounts.getTokenManager(connection, tokenManagerId),
...
export const withTransfer = async (transaction, connection, wallet, mintId, recipient = wallet.publicKey) => {
    const tmManagerProgram = tokenManagerProgram(connection, wallet);
    ...
    const recipientTokenAccountId = await withFindOrInitAssociatedTokenAccount(transaction, connection, mintId, recipient, wallet.publicKey, true);
    const remainingAccountsForKind = getRemainingAccountsForKind(mintId, tokenManagerData.parsed.kind);
    const remainingAccountsForTransfer = getRemainingAccountsForTransfer(tokenManagerData.parsed.transferAuthority, tokenManagerId);
...
export const withDelegate = async (transaction, connection, wallet, mintId, recipient = wallet.publicKey) => {
    const tmManagerProgram = tokenManagerProgram(connection, wallet);
    const tokenManagerId = findTokenManagerAddress(mintId);
    const tokenManagerData = await tryGetAccount(() => getTokenManager(connection, tokenManagerId));
    if (!(tokenManagerData === null || tokenManagerData === void 0 ? void 0 : tokenManagerData.parsed)) {
...
export const withUndelegate = async (transaction, connection, wallet, mintId, recipient) => {
    const tmManagerProgram = tokenManagerProgram(connection, wallet);
    const tokenManagerId = findTokenManagerAddress(mintId);
    const tokenManagerData = await tryGetAccount(() => getTokenManager(connection, tokenManagerId));
    if (!(tokenManagerData === null || tokenManagerData === void 0 ? void 0 : tokenManagerData.parsed)) {
    ...
    const mintManagerId = findMintManagerId(mintId);
    const recipientTokenAccountId = await findAta(mintId, recipient !== null && recipient !== void 0 ? recipient : wallet.publicKey, true);
...
export const withSend = async (transaction, connection, wallet, mintId, senderTokenAccountId, target) => {
    const tmManagerProgram = tokenManagerProgram(connection, wallet);
    const tokenManagerId = findTokenManagerAddress(mintId);
    const tokenManagerData = await tryGetAccount(() => getTokenManager(connection, tokenManagerId));
    if (!(tokenManagerData === null || tokenManagerData === void 0 ? void 0 : tokenManagerData.parsed)) {
...
export const withMigrate = async (transaction, connection, wallet, mintId, rulesetName, holderTokenAccountId, authority) => {
    const tmManagerProgram = tokenManagerProgram(connection, wallet);
...
export const withReplaceInvalidator = async (transaction, connection, wallet, tokenManagerId, newInvalidator) => {
    const tmManagerProgram = tokenManagerProgram(connection, wallet);
