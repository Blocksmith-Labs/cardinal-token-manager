...
import { CRANK_KEY, InvalidationType, TokenManagerKind, TokenManagerState, } from "./programs/tokenManager";
...
import { findMintManagerId, findTokenManagerAddress, tokenManagerAddressFromMint, } from "./programs/tokenManager/pda";
import { getRemainingAccountsForKind, getRemainingAccountsForTransfer, withRemainingAccountsForPayment, withRemainingAccountsForReturn, } from "./programs/tokenManager/utils";
 ...
 */
export const withIssueToken = async (transaction, connection, wallet, { claimPayment, timeInvalidation, useInvalidation, mint, issuerTokenAccountId, amount = new BN(1), transferAuthorityInfo, kind = TokenManagerKind.Managed, invalidationType = InvalidationType.Return, visibility = "public", permissionedClaimApprover, receiptOptions = undefined, customInvalidators = undefined, }, payer = wallet.publicKey) => {
    // create mint manager
    if (kind === TokenManagerKind.Managed ||
        kind === TokenManagerKind.Permissioned) {
        const [mintManagerIx, mintManagerId] = await tokenManager.instruction.creatMintManager(connection, wallet, mint, payer);
    ...
    const [tokenManagerIx, tokenManagerId] = await tokenManager.instruction.init(connection, wallet, mint, issuerTokenAccountId, amount, kind, invalidationType, numInvalidator, payer);
    ...
    if (transferAuthorityInfo) {
        ...
        transaction.add(setTransferAuthority(connection, wallet, tokenManagerId, checkTransferAuthority.pubkey));
        if (transferAuthorityInfo.setInvalidator) {
            transaction.add(tokenManager.instruction.addInvalidator(connection, wallet, tokenManagerId, checkTransferAuthority.pubkey));
        }
    }
    //////////////////////////////
    /////// claim approver ///////
    ...
    if (claimPayment) {
        ...
        const [paidClaimApproverIx, paidClaimApproverId] = await claimApprover.instruction.init(connection, wallet, tokenManagerId, claimPayment, payer);
        ...
        transaction.add(tokenManager.instruction.setClaimApprover(connection, wallet, tokenManagerId, permissionedClaimApprover));
    }
    //////////////////////////////
    /////// time invalidator /////
    //////////////////////////////
    if (timeInvalidation) {
        const [timeInvalidatorIx, timeInvalidatorId] = await timeInvalidator.instruction.init(connection, wallet, tokenManagerId, timeInvalidation, payer);
    ...
    //////////////////////////////
    if (useInvalidation) {
        const [useInvalidatorIx, useInvalidatorId] = await useInvalidator.instruction.init(connection, wallet, tokenManagerId, useInvalidation, payer);
    ...
    transaction.add(tokenManager.instruction.issue(connection, wallet, tokenManagerId, tokenManagerTokenAccountId, issuerTokenAccountId, payer, kind === TokenManagerKind.Permissioned
        ? [
            {
                pubkey: CRANK_KEY,
                isSigner: false,
    ...
    if (receiptOptions) {
        ...
        transaction.add(await tokenManager.instruction.claimReceiptMint(connection, wallet, "receipt", tokenManagerId, receiptMintKeypair.publicKey, payer));
    }
...
export const withClaimToken = async (transaction, connection, wallet, tokenManagerId, additionalOptions) => {
    ...
    if (claimApproverData &&
        ...
        const paymentAccounts = await withRemainingAccountsForPayment(transaction, connection, wallet, tokenManagerData.parsed.mint, claimApproverData.parsed.paymentMint, tokenManagerData.parsed.issuer, claimApproverData.parsed.paymentManager, {
        ...
        const [createClaimReceiptIx, claimReceipt] = await tokenManager.instruction.createClaimReceipt(connection, wallet, tokenManagerId, tokenManagerData.parsed.claimApprover, additionalOptions === null || additionalOptions === void 0 ? void 0 : additionalOptions.payer);
    ...
    const recipientTokenAccountId = await withFindOrInitAssociatedTokenAccount(transaction, connection, tokenManagerData.parsed.mint, wallet.publicKey, (_a = additionalOptions === null || additionalOptions === void 0 ? void 0 : additionalOptions.payer) !== null && _a !== void 0 ? _a : wallet.publicKey);
    // claim
...
export const withExtendExpiration = async (transaction, connection, wallet, tokenManagerId, secondsToAdd, options) => {
    const [timeInvalidatorId] = await timeInvalidator.pda.findTimeInvalidatorAddress(tokenManagerId);
    ...
    if (timeInvalidatorData && timeInvalidatorData.parsed.extensionPaymentMint) {
        ...
        const paymentAccounts = await withRemainingAccountsForPayment(transaction, connection, wallet, tokenManagerData.parsed.mint, timeInvalidatorData.parsed.extensionPaymentMint, tokenManagerData.parsed.issuer, timeInvalidatorData.parsed.paymentManager, {
            receiptMint: tokenManagerData.parsed.receiptMint,
...
export const withExtendUsages = async (transaction, connection, wallet, tokenManagerId, usagesToAdd, options) => {
    const [useInvalidatorId] = await useInvalidator.pda.findUseInvalidatorAddress(tokenManagerId);
    const [useInvalidatorData, tokenManagerData] = await Promise.all([
    ...
    if (useInvalidatorData && useInvalidatorData.parsed.extensionPaymentMint) {
        ...
        const paymentAccounts = await withRemainingAccountsForPayment(transaction, connection, wallet, tokenManagerData.parsed.mint, useInvalidatorData.parsed.extensionPaymentMint, tokenManagerData.parsed.issuer, useInvalidatorData.parsed.paymentManager, {
            receiptMint: tokenManagerData.parsed.receiptMint,
            payer: options === null || options === void 0 ? void 0 : options.payer,
        });
...
export const withUpdateMaxExpiration = async (transaction, connection, wallet, tokenManagerId, newMaxExpiration) => {
    ...
    if (tokenManagerData.parsed.state !== TokenManagerState.Invalidated) {
        transaction.add(timeInvalidator.instruction.updateMaxExpiration(connection, wallet, timeInvalidatorId, tokenManagerId, newMaxExpiration));
    }
    else {
...
export const withTransfer = async (transaction, connection, wallet, mintId, recipient = wallet.publicKey) => {
    ...
    const remainingAccountsForTransfer = await getRemainingAccountsForTransfer(tokenManagerData.parsed.transferAuthority, tokenManagerId);
    transaction.add(tokenManager.instruction.transfer(connection, wallet, tokenManagerId, mintId, tokenManagerData.parsed.recipientTokenAccount, recipient, recipientTokenAccountId, [...remainingAccountsForKind, ...remainingAccountsForTransfer]));
    return transaction;
};
export const withDelegate = async (transaction, connection, wallet, mintId, recipient = wallet.publicKey) => {
    ...
    transaction.add(tokenManager.instruction.delegate(connection, wallet, mintId, tokenManagerId, mintManagerId, recipient, tokenManagerData.parsed.recipientTokenAccount));
    return transaction;
};
export const withUndelegate = async (transaction, connection, wallet, mintId, recipient) => {
    ...
    transaction.add(tokenManager.instruction.undelegate(connection, wallet, mintId, tokenManagerId, mintManagerId, recipient !== null && recipient !== void 0 ? recipient : wallet.publicKey, recipientTokenAccountId));
    return transaction;
};
export const withSend = async (transaction, connection, wallet, mintId, senderTokenAccountId, target) => {
    ...
    transaction.add(tokenManager.instruction.send(connection, wallet, mintId, tokenManagerId, mintManagerId, wallet.publicKey, senderTokenAccountId, target, targetTokenAccountId));
    return transaction;
};
