...
export const init = async (connection, wallet, tokenManagerId, params, payer = wallet.publicKey) => {
    ...
    const [claimApproverId, _claimApproverBump] = await findClaimApproverAddress(tokenManagerId);
...
export const pay = async (connection, wallet, tokenManagerId, payerTokenAccountId, paymentManager, paymentAccounts) => {
    const provider = new AnchorProvider(connection, wallet, {});
    const claimApproverProgram = new Program(CLAIM_APPROVER_IDL, CLAIM_APPROVER_ADDRESS, provider);
    const [claimReceiptId, _claimReceiptBump] = await findClaimReceiptId(tokenManagerId, wallet.publicKey);
