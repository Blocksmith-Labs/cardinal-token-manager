...
const getLink = (
  ...
  if (otp) {
    url.searchParams.append(
      "otp",
      anchor_1.utils.bytes.bs58.encode(otp.secretKey)
    );
  }
  if (cluster === "devnet") {
  ...
  try {
    ...
    return [
      ...
      web3_js_1.Keypair.fromSecretKey(
        anchor_1.utils.bytes.bs58.decode(otpRegex[1])
      ),
    ];
  } catch (e) {
...
const issueToken = async (
  ...
  const [transaction, tokenManagerId, otp] = await (0, _1.withIssueToken)(
...
const claimFromLink = async (connection, wallet, tokenManagerId) =>
  (0, _1.withClaimToken)(
    new web3_js_1.Transaction(),
    connection,
    wallet,
